  ...
  <div class="my-filter-content">
    <div class="my-fc-search">
      <div class="my-fc-search-top">
        <vxe-input v-model="option.data.sVal" placeholder="搜索" @input="searchEvent" />
      </div>
        ...
        <template v-if="valList.length">
          ...
          <ul class="my-fc-search-list my-fc-search-list-body">
            <li v-for="(item, sIndex) in valList" :key="sIndex" class="my-fc-search-item">
              ...
              >{{ item.value === '' ? '[空白]' : item.value }} ({{item.count}})</vxe-checkbox>
            </li>
          </ul>
        </template>
        <template v-else>
...
export default {
  ...
  methods: {
    ...
    load() {
      ...
      const colValList = Object.keys(groupData)
        .map((val) => {
          return {
